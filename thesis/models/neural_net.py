...
class EuclideanDistanceClassifier(nn.Module):
    #idea get layer_realtion with cosine_similarity
    def __init__(self, num_llm_layers, embedding_size):
        super().__init__()
...
class Baseline1(nn.Module):
    #1 token_encoder for every layer 
    #1 seperate classifier on top of it 
    def __init__(self, num_llm_layers, embedding_size):
        super().__init__()
        ...
        #model architecture
        self.token_encoder = nn.ModuleList([nn.Linear(embedding_size,hidden_size) for _ in range(num_llm_layers)])
        self.classifiers = nn.ModuleList([nn.Linear(hidden_size,1) for _ in range(num_llm_layers)])
        self.activation = nn.ReLU()
        self.final_classifier = nn.Linear(num_llm_layers,1)

    def forward(self, x,return_encoded_space=False):
        #x = [batch, num_layers, embedding_size]
        encoded_space = [self.activation(layer(x[:,i])) for i,layer in enumerate(self.token_encoder)]
        results = [classifier(encoded) for encoded,classifier in zip(encoded_space,self.classifiers)]
        
        result = self.final_classifier(torch.cat(results,dim=-1))
        
        if return_encoded_space:
            return results,encoded_space
        return result, None        

class Baseline2(nn.Module): 
    # concat all layers then use one token_encoder only 
    # 1 classifier on top of it
    ...
    def __init__(self, num_llm_layers, embedding_size):
        ...
        #model architecture
        self.token_encoder = nn.Linear(embedding_size*num_llm_layers,hidden_size)
        self.classifier = nn.Linear(hidden_size,1)
        self.activation = nn.ReLU()    
    
    def forward(self, x,return_encoded_space=False):
        #x = [batch, num_layers, embedding_size]
        encoded_space = self.activation(self.token_encoder(x.view(x.size(0),-1)))
        result = self.classifier(encoded_space)
...
if __name__ == "__main__":
    ...
    model = EuclideanDistanceClassifier(layer_size,embedding_size)
   ...
   # model.plot_classifier_weights()
    ...
    #test baselines
    model = Baseline1(layer_size,embedding_size)
    print_number_of_parameters(model)
    
    modle = Baseline2(layer_size,embedding_size)
    print_number_of_parameters(model)
